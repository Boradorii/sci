...
conn = pymysql.connect(host='localhost', db ='smart_g', user='edlmanager', password='Sci2019qa!')
...
def user_batch_check(userCode):
    try:
        ...
        url = "http://192.168.3.154:9000/api/batch/B2C_Batch"
...
def data_backup(userCode):
    try:
        ...
        url = "http://192.168.3.154:9000/api/batch/data_backup"
        response = requests.post(url=url, data=params)
        result= response.json()
        return result
...
def batch_result_insert(userCode, sleep_hr, sleep_stress, sleep_index, glucos, pressure_high, pressure_low, health_index):
    try :
        ...
        url = "http://192.168.3.154:9000/api/batch/batch_insert"
        response = requests.post(url=url, data=params)
...
class predictDisease:
    def __init__(self):
        ...
        #############################################################################################################################################################
        # # 혈당 예측 모델
        # self.diabetespredmodel = tf.keras.models.load_model('C:\\Users\\SCI\\Desktop\\B2C(2.0)\\server\\config\\batch\\predict\\B2C_Model\\bloodsugar_pre_0125.h5')  
        # self.diabetespredsc=load('C:\\Users\\SCI\\Desktop\\B2C(2.0)\\server\\config\\batch\\predict\\B2C_Model\\bloodsugar_pre_0125.bin')  

        # # 최고 혈압 예측 모델
        # self.highbppredict=tf.keras.models.load_model('C:\\Users\\SCI\\Desktop\\B2C(2.0)\\server\\config\\batch\\predict\\B2C_Model\\BP_HIGH_0112.h5')  
        # self.highbppredictsc=load('C:\\Users\\SCI\\Desktop\\B2C(2.0)\\server\\config\\batch\\predict\\B2C_Model\\BP_HIGH_0112.bin')  

        # # 최저 혈압 예측 모델
        # self.lowbppredict=tf.keras.models.load_model('C:\\Users\\SCI\\Desktop\\B2C(2.0)\\server\\config\\batch\\predict\\B2C_Model\\BP_LOW_0113.h5')  
        # self.lowbppredictsc=load('C:\\Users\\SCI\\Desktop\\B2C(2.0)\\server\\config\\batch\\predict\\B2C_Model\\BP_LOW_0113.bin')  

        # 혈당 예측 모델
        self.diabetespredmodel = tf.keras.models.load_model('/home/server/B2C_server/config/batch/predict/B2C_Model/glucose/blood_sugar_20230308_300_gen_del_6.h5')  
        self.diabetespredsc=load('/home/server/B2C_server/config/batch/predict/B2C_Model/glucose/blood_sugar_20230315_350_gen_del.bin')  

        # 최고 혈압 예측 모델
        self.highbppredict=tf.keras.models.load_model('/home/server/B2C_server/config/batch/predict/B2C_Model/bp_high/BP_HIGH_230314_gen_del_1.h5')  
        self.highbppredictsc=load('/home/server/B2C_server/config/batch/predict/B2C_Model/bp_high/BP_HIGH_230314_gen_del_2.bin')  

        # 최저 혈압 예측 모델
        self.lowbppredict=tf.keras.models.load_model('/home/server/B2C_server/config/batch/predict/B2C_Model/bp_low/blood_pressure_20230315_350_gen_del_3.h5')  
        self.lowbppredictsc=load('/home/server/B2C_server/config/batch/predict/B2C_Model/bp_low/blood_pressure_20230315_350_gen_del_2.bin')
        #############################################################################################################################################################
