 ...
 *  ================================================================
 */
const authTokenUrl = baseUrl + '/token';
const retryLoginUrl = serverUrl + 'error/auth?';

function emergencySong() {
...
function fallSong() {
    ...
    sourceOgg.src = 'http://scikorea.co.kr/sound/fall.ogg';
    sourceMp3.src = 'http://scikorea.co.kr/sound/fall.mp3';
...
function changeView(pageUrl, reqData) {
    ...
    let changePageUrl, nowPage;
    let userCode = getSessionStorage('h_adminCode'); // SY 수정_230622
    let failPage = retryLoginUrl + 'userCode=' + getSessionStorage('h_adminCode');
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (succ == 2) { // accessToken이 갱신되면
                ...
                nowPage = getSessionStorage('culPageUrl')
                console.log(nowPage)
...
function setHourSelBox($selBox) {
    let optStr = '';
    for (let i = 0; i <= 23; i++) {
        if (i < 10) {
            optStr += '<option value="0' + i + '">0' + i + '시</option>';
        } else {
            optStr += '<option value="' + i + '">' + i + '시</option>';
        }
    }
    $selBox.append(optStr);
    $selBox.val('00').prop('selected', true);
}
/**
 * 분 select box option 생성하기
 *
 * @params $selBox - option을 추가하기 위한 기준 selectbox 요소(jquery 객체)
 * @author MiYeong Jang
 * @since  2022.03.29
 */
function setMinSelBox($selBox) {
    let optStr = '';
    for (let i = 0; i < 60; i += 5) {
        if (i < 10) {
            optStr += '<option value="0' + i + '">0' + i + '분</option>';
        } else {
            optStr += '<option value="' + i + '">' + i + '분</option>';
        }
    }
    $selBox.append(optStr);
    $selBox.val('00').prop('selected', true);
...
function setMinSelBox2($selBox) {
    ...
    for (let i = 0; i <= 60; i += 5) {
        if (i < 10) {
            ...
            optStr += '<option value="' + i + '">59분</option>';
        }
